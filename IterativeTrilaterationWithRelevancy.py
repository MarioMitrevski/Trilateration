...
def trilaterate_2d(node_for_trilaterate: Node, anchor_neighbour_nodes):
    ...
    node_for_trilaterate.relevancy = sum([three_anchor_neighbours[i][0].relevancy for i in range(3)], 1)
...
def find_distance_with_error(node_to_localize, anchor):
    if anchor.is_anchor:
        ...
        euclidean_distance = find_euclidean_distance(node_to_localize.x, node_to_localize.y, anchor.x_prim,
                                                     anchor.y_prim)
...
def localize_node(node_to_localize: Node):
    ...
    sorted_neighbour_anchor_nodes = sorted(
        ...
        key=lambda t: t[0].relevancy
    )
    print(sorted_neighbour_anchor_nodes)
    trilaterate_2d(node_to_localize, sorted_neighbour_anchor_nodes)


def localize_sensor_nodes():
    number_of_localized = 0
    while number_of_localized < N:
        for sensor_node in sensor_nodes:
            if not sensor_node.is_found:
                localize_node(sensor_node)

        for sensor_node in sensor_nodes:
            if sensor_node.is_found:
                number_of_localized += 1
...
def get_error_of_localization():
    sum_of_distances = 0
    for node in sensor_nodes:
        if node.is_found and not node.is_anchor:
...
if __name__ == '__main__':
    ...
    L = 100  # plot size
    R = 10  # radio range from 20 to 50
    ...
    plt.ylabel(f'Average localization error in percentage from {N} nodes')
